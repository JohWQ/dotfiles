...
arch() {
  ...
  search='pacman'
  [[ -n "$1" ]] && search=$(select_from 'yay' 'pacaur' 'trizen' 'yaourt' 'packer' 'apacman' 'pamac' 'pacman')
...
fedora() {
  fzf --query="$*" --preview='p={}; p="${p%% *}"; dnf -q info "${p%.*}"' \
    < <(dnf -qC repoquery '*' | sed 's/-[0-9].*//' | rg -v "kernel|ghc|golang-github|kf5|kf6|texlive") |
...
if [[ -r /etc/os-release ]]; then
  # Source /etc/os-release safely to read ID and ID_LIKE
